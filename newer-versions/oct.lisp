...
;;;  Now has fft, polynomial eval, quadrature
...
;; (r "sqrt")   take care of complex version. see later.
...
(defmethod sqrt((x oct))
  (if (< (octi::oct-0 (oct-real x)) 0d0) ;; negative answer.
      (make-octz :imag (sqrt (- x)))
     (make-oct :real (octi::sqrt-oct (oct-real x)))))
    
;;; need to allow for octz+oct, oct+octz, octz+real real+octz  etc.
;;; whew, macro writing can do all this too
...
;; example  (polyeval (list (into 1d0)(into 2d0)(into 3d0))(into 2d0))
...
;; example  (polyevald '(1d0 2d0 3d0)2d0) is 11d0
...
(defun gaussab (fun lo hi n) ;; integrate from lo to hi
  (let ((a (* 1/2 (- hi lo)))
    ...
    (* a (int_gl #'(lambda(r)(funcall fun (+ b (* a r))))
		 n))))
...
;; Here is the  tanh/sinh method. This also integrates from -1 to 1.

(defun quadts(fun n)
  (declare (special octpi/2))
  (let*(
	(h (into(/ 4 n)))
	(sum (* octpi/2 (funcall fun 0)))
	(he (exp h))
	(relerr(into #.(expt 2.0d0 (- 50))))
	(t2 1)t3 t4 ab cor (oldsum (into 0d0)))
    (loop for j from 1 to (* 2 n) do
	  (setf t2 (* t2 he))
	  (setf t3 (exp (* octpi/2 (/ (- (* t2 t2) 1)
				    (* t2 2)))))
	  (setf ab (* t3 t3))
	  (setf t4 (/ (+ ab 1)(* 2 t3)))
...
(defun test2 ()
  ;;    ;;pi via Machin's atan formula~
  ;; pi/4 = 4 * arctan(1/5) - arctan(1/239)
  (* (- (* (atan (into 1/5)) 4) (atan(into 1/239))) 4))
