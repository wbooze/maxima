...
;; an attempt to get FFT to run fast by removing tagging of aqd's etc.
;; this works, but is hackish.
;; for stuff that works, and maybe more comments, look at qd.lisp file.
	...
	(case argnum
	  ...
	  (2
	   `(progn
	      ;;     (assert (aqd-p ,targ)) 
	      (let (
		    (a1 (aqd-q ,(macroexpand `(with-temps ,(car args)))))
		   ;(a1 (aqd-q  ,(car args)))
...
(defun newzero()  (make-array  4 :element-type 'double-float
			      :allocation :lispstatic-reclaimable
			      :initial-element 0.0d0  ))

;;; omit headers for aqd.
(defun v2dfa(a &optional (m (length a))	)
  ...
  (let* ((k (length a))
    ...
    (dotimes (i k)			;just copy the actual numbers, or convert?
	 ...
	 (if (arrayp (setf h (aref a i))) h (aqd-q(qd::into h)))) ;; here we convert.
...
(defun dfa2v(a &optional (m (/ (length a)2)))
  ...
  (let* ((k (/ (length a) 2))
    ...
    (dotimes (i m ans)
      (declare (fixnum i))
      (setf (aref ans i)(round (ga::outof (make-aqd :q (aref a (cl::* 2 i)))) k))))))
...
(defun polymultfftqd(r s)
  ...
  (let* ((lr (length r))
    ...
    (prodarray rfft sfft z ans)  ;; put the answer in ans
    (dfa2v(four1 ans z :isign -1) lans)
    ))


(defparameter t1 (newzero))
(defparameter t2 (newzero))
(defparameter t3 (newzero))
(defparameter place-e (newzero))
(defparameter place-o (newzero))
...
(defun prodarray(r s len ans)
  ...
  (let ();;((ans (make-array (* 2 len))))
    ...
    (let* ((ind (cl::* 2 i))
      ...
      (qd_mul a c t1)
      (qd_mul b d t2)
      (qd_sub t1 t2 place-e)
      (qd_mul a d t1)
      (qd_mul b c t2)
      (qd_add t1 t2 place-o)
      (qd_copy_into place-e  (aref ans ind))
      (qd_copy_into place-o  (aref ans ind1))
    ans
	  ))))

(defparameter qdtwopi
   (aqd-q (ga::/ (qd::into  165424160919322423196824703508232170249081435635340508251270944637 )
...
(defparameter minone (aqd-q (qd::into -1)))
...
;; not using aqds but just raw arrays of floats
...
(defun four1 (data nn &key (isign 1))
  ...
  (prog ((wr (newzero)) 
	...
	(when (cl::> n mmax)
	  ...
	  (setf (aref theta 0)(coerce (cl::* isign mmax) 'double-float))
	  (setf (aref theta 1) 0.0d0)
	  (setf (aref theta 2) 0.0d0)
	  (setf (aref theta 3) 0.0d0)
	  ;;(dsetv theta(with-temps (/ qdtwopi theta)))
	  (qd_div qdtwopi theta theta)
	  

	  ;; (dsetv halftheta(with-temps (* 1/2 theta)))
	  (qd_mul onehalf theta halftheta)
	  (qd_sincos halftheta wpr cost)
	  (qd_mul wpr cost cost)
	  (qd_mul cost two wpi)
	  ;; (dsetv wpi (with-temps (* 2(* wpr cost)))) ;; 2*sin(t/2)*cos(t/2)
	  (qd_mul wpr wpr wpr)
	  (qd_mul mintwo wpr wpr)
	  ;;(dsetv wpr (with-temps (* -2 (* wpr wpr))))

	  ;;(dsetv wr one)
	  
  	  (setf (aref wr 0) 1.0d0)
	  (setf (aref wr 1) 0.0d0)
	  (setf (aref wr 2) 0.0d0)
	  (setf (aref wr 3) 0.0d0)

	  ;;(dsetv wi zero)
	  (setf (aref wi 0) 0.0d0)
	  (setf (aref wi 1) 0.0d0)
	  (setf (aref wi 2) 0.0d0)
	  (setf (aref wi 3) 0.0d0)
	  ...
	  (do ((m 1 (cl::+ m 2)))
		...
		((cl::> i n) t)
	      ...
	      ;;			  (* wi (aref data j)))))
	      (qd_mul wr (aref data (cl::1- j)) t1)
	      (qd_mul wi (aref data j) t2)
	      (qd_sub t1 t2 tempr)
	      
	      ;;     (dsetv tempi (with-temps (+ (* wr (aref data j)) (* wi (aref data (cl::1- j))))))
	      (qd_mul wr (aref data j) t1)
	      (qd_mul wi (aref data (cl::1- j)) t2)
	      (qd_add t1 t2 tempi)
	      
	      ;; (dsetv (aref data (cl::1- j))(- (aref data (cl::1- i)) tempr))
	      (qd_sub  (aref data (cl::1- i)) tempr (aref data (cl::1- j)))
	      
	      ;;(dsetv (aref data j)(- (aref data i) tempi))
	      (qd_sub  (aref data i) tempi (aref data j))
	      
	      ;;(dsetv (aref data (cl::1- i)) (+ (aref data (cl::1- i)) tempr))
	      (qd_add  (aref data (cl::1- i)) tempr (aref data (cl::1- i)))
	      
	      ;;(dsetv (aref data i) (+  (aref data i) tempi))
	      (qd_add  (aref data i) tempi (aref data i)) )
	      ;;(dsetv wtemp wr)
	      (qd_copy_into wr wtemp)
...
(setf qrn (map 'vector #'aqd-q qr))
(time (progn (polymultfftqd qrn qrn) nil))
...
#|  Important timing results regarding overhead of aqd wrapping.

;;NO AQD WRAPPING of 


 (time (dotimes (i 10)(progn (polymultfftqd qrn qrn) nil))) ;; on windsor no wrapping.
; cpu time (non-gc) 2,143 msec user, 0 msec system
; cpu time (gc)     170 msec user, 0 msec system
; cpu time (total)  2,313 msec user, 0 msec system
; real time  2,524 msec
; space allocation:
;  92,678 cons cells, 21,913,752 other bytes, 2,493,180 static bytes
;; windsome, a 2.5GHz pentium 4 is about the same.. .. 2,578 ms.

;; with AQD wrapping
(time (dotimes (i 10)(progn (polymultfftqd qr qr) nil))) ; on windsor
; cpu time (non-gc) 2,634 msec user, 0 msec system
; cpu time (gc)     591 msec user, 0 msec system
; cpu time (total)  3,225 msec user, 0 msec system
; real time  3,375 msec
; space allocation:
;185,182 cons cells, 28,758,256 other bytes, 3,746,240 static bytes

;; windsome, a 2.5GHz pentium 4 is only a little faster .. 2,975 ms.

nil
So, 25% faster with no wrapping, on windsor, pentium 3.
more like 15% faster on pentium 4.
|#