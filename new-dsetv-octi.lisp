...
(eval-when (load compile eval)
  (mapc #'(lambda(r)
	...
	'((+ two-arg-+ add-oct-t)
	  ...
	  (/ two-arg-/ div-oct-t)
	  (excl::/_2op two-arg-/ div-oct-t) ;;etc
...
(defun copy-octi(inf int); from two
...
(block nil
  (let ((i (into 1)))
    (tagbody
      #:Tag199
        (if (>-oct-t i #(10.0d0 0.0d0 0.0d0 0.0d0)
                    #(1.0d0 0.0d0 0.0d0 0.0d0))
            (progn (return-from nil (progn 'foo)))
          nil)
        (tagbody (add-oct-t w i w))
        (let ()
          (setq i
                (add-oct-t i #(1.0d0 0.0d0 0.0d0 0.0d0)
                          #(1.0d0 0.0d0 0.0d0 0.0d0)))
          nil)
        (go #:Tag199))))
...
(defun string2octi(s);; read integerQinteger like 10Q2   = 100 = 0.1Q3  or  -3.1q-1
  ...
  (let ((p0 0)(sign (into 1)))
    ...
    (if (char= #\- (aref s 0))(setf p0 (into 1) sign(into  -1)));; set sign if negative
  ...
  ;;    (format t "~% frac= ~s pos=~s" frac pos)
      ...
      (if (= pos (length s)) (mul-oct sign (into frac))
	...
	;;     (format t "~% sign=~s frac=~s expon=~s" sign frac expon)
	     (mul-oct sign (into frac) (into (expt 10 expon)))))
	  (#\.;; 
...
(pprint (macroexpand '(with-temps (do ((i 1  (1+ i)) (f 1  (* f i)))((> i x) f)))))
	...
	((genlist(n) (loop for i from 1 to n collect (into i)))
	 ...
	 (maketemps(r)			;change r=(+ a (* b c)) to  temp storage .
	   ...
	   (cond ((numberp r) (into r))
		 ...
		 ((member (car r) '(block tagbody if when))
		  (mapcar #'(lambda(x)(wt x)) r ))
	       
		 ((member (car r)'(go make-array copy-if-oct return-from 
				   add-oct-t mul-oct-t sub-oct-t div-oct-t));; etc
		   r) 
		 
		 ((member (car r) '( setq setf psetq))
		  (cons (car r)		;  expand everything (set* name ..); protect via copy
			(cons (cadr r)
			      (mapcar #'(lambda(x)
					  `(copy-if-oct ,(wt x)))
				      (cddr r))))) 
		 ((member (car r) '( progn))
		  (cons (car r)		;  expand everything after progn
			...
			(mapcar #'wt (cdr r)   )))
		 ((arrayp r) r)
		 (t  (cons (car r)	;  expand everything after unknown function, (foo ...)
