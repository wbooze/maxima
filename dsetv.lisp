...
(eval-when (load compile eval)
  (mapc #'(lambda(r)
	...
	;; might appear, after macro-expansion. We hope that if just + is used,
	;; it is used with 2 args, otherwise an error will be signalled.
...
(defmacro dsetv
  ...
  (cond 
   ...
   ;; ((eq (car ex) 'incf) (error "incf?")) ;; incf will never happen. It is macroexpanded to +
   ;;  change (incf x) to (dsetv x (+ x 1))
   ;;  (cond ((= (length ex) 2) `(dsetv ,(cadr ex) (+ ,(cadr ex) 1)))
   ;;  change (incf x y)(to (dsetv x (+ x y))
   ;;	  ((= (length ex) 3) `(dsetv ,(cadr ex) (+ ,(cadr ex) ,(caddr ex))))
   ;;	  (t (error "incf was given wrong number of args, ~s" ex))))
   ...
   (t 
    (let* ((op (car ex))
	   ...
	   (theval (macroexpand `(with-temps (,op ,@ args))))
	   )
      (cond 	       
			      ...
			      (cons (car r); let or let*
				    (cons (mapcar 
					   #'(lambda(x); pair: name, value
					       (list (car x); variable
						     (macroexpand `(with-temps ,(cadr x)))))
					   (cadr r))
					  (mapcar #'(lambda(x)
						      (macroexpand `(with-temps ,x)))(cddr r))
